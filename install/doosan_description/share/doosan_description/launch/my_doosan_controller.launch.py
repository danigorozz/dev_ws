...
def generate_launch_description():
	...
	xacro_file = get_package_share_directory('doosan_description') + '/description'+'/xacro/'+ robot_model +'.urdf.xacro'
	...
	#world = os.path.join(get_package_share_directory('doosan_description'), 'worlds', world_file_name)
	#gazebo_node = ExecuteProcess(cmd=['gazebo', '--verbose', world,'-s', 'libgazebo_ros_factory.so'], output='screen')


	'''
	# RViz
	rviz_config_file = get_package_share_directory('doosan_description') + "/rviz/view_config.rviz"
